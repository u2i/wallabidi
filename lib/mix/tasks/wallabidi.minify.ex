defmodule Mix.Tasks.Wallabidi.Minify do
  ...
  def run(_args) do
    ...
    unless System.find_executable("esbuild") do
      Mix.raise("esbuild not found in PATH — install via `brew install esbuild` or your package manager")
    end
    ...
    case System.cmd("esbuild", ["--minify", "--target=es2020", @src], stderr_to_stdout: true) do
      {output, 0} ->
        ...
        Mix.shell().info("#{@src}: #{before_size} bytes -> #{@dst}: #{after_size} bytes (#{ratio}%)")
