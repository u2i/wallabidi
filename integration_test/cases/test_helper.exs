...
# When WALLABIDI_POOL_SIZE is set, start a Wallabidi.Pool of that size
# for the active driver and route Wallabidi.start_session through it.
# Default behavior (env var unset) is the legacy direct-start path —
# every existing test runs unchanged.
case System.get_env("WALLABIDI_POOL_SIZE") do
  nil ->
    :ok
  ...
  size_str ->
    ...
    if impl do
      pool_name = :"wallabidi_pool_#{driver}"
      ...
      Application.put_env(:wallabidi, :pools, %{driver => pool_name})

      IO.puts("[test_helper] session pool active: driver=#{driver} size=#{pool_size}")
    else
      IO.puts("[test_helper] WALLABIDI_POOL_SIZE set but driver #{driver} has no pool impl — skipping")
    end
end
